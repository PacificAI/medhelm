"""Test suite for PS26 Breach Response Scenario CSV validation.
...
from helm.benchmark.scenarios.ps26_breach_response_scenario import PS26BreachResponseScenario
...
def _valid_row(**overrides) -> dict:
    ...
    row = {
        "id": "test_001",
        "prompt": "What is HIPAA breach notification requirement?",
...
def _make_scenario_with_csv(rows: List[dict], tmpdir: str) -> PS26BreachResponseScenario:
    """Create a scenario instance with a CSV file containing the given rows."""
    csv_path = os.path.join(tmpdir, "data.csv")
    _write_csv(csv_path, rows)
    return PS26BreachResponseScenario(data_path=csv_path)
...
def test_valid_csv_loads_single_row():
    ...
    with TemporaryDirectory() as tmpdir:
        ...
        assert instances[0].input.text == "What is HIPAA breach notification requirement?"
...
def test_missing_required_column_raises_clear_error(missing_col):
    ...
    with TemporaryDirectory() as tmpdir:
        ...
        scenario = PS26BreachResponseScenario(data_path=csv_path)
